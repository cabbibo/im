...
SubShader
    {
        ...
        float _TerrainHeight;
            ...
            varyings vert(input v)
            {
                ...
                o.rR  = ComputeScreenPos(mul(UNITY_MATRIX_VP, float4(pos + refrR * .3,1)));
                o.rG  = ComputeScreenPos(mul(UNITY_MATRIX_VP, float4(pos + refrG * .3,1)));
                o.rB  = ComputeScreenPos(mul(UNITY_MATRIX_VP, float4(pos + refrB * .3,1)));
...
float smin( float a, float b, float k )
{
    ...
    return min( a, b ) - h*h*0.25/k;
            ...
            float4 frag(varyings v) : COLOR
            {
                ...
                float pDif  = length( _Player.xz - v.world.xz);
                ...
                float2 difP = _Player.xz - v.world.xz;
                ...
                float4 c = tex2Dlod(_HeightMap , float4(v.world.xz * _TerrainSize,0,0) );
                
                float dif = v.world.y - c.r * _TerrainHeight;
                ...
                col = tex2D(_DepthRampTex,float2(bg.b * .2 + .7,0)  ) + aroundPerson ;//tCol;//refl * .5 + .5;
